...
"color:#ffffff;")
        ...
        self.graphView.setGeometry(QtCore.QRect(700, 170, 500, 431))
        self.graphView.setMinimumSize(QtCore.QSize(500, 431))
        self.graphView.setMaximumSize(QtCore.QSize(500, 431))
    ...
    def printGraph(self):
        ...
        if(int(time.strftime("%d/%m/%Y").split("/")[1]) == 2):
        	if(((current_year % 4 == 0 && current_year % 100 != 0) || current_year % 400 == 0)):
        		month_len = self.monthList[int(str(time.strftime("%d/%m/%Y").split("/")[1]))] + 1 #anno bisestile	
        	else:
        		month_len = self.monthList[int(str(time.strftime("%d/%m/%Y").split("/")[1]))]
        ...
        for el in month_import_list:
        	el = 0

        for period in dataSet:
        	if(str(time.strftime("%d/%m/%Y").split("/")[1]) == str(period[3].split("/")[1])):
        		month_import_list[int(period[3].split("/")[0])] = float(period[2]) #nella lista assegno all'indice che prende il valore del giorno l'importo corrispondente

        #riempio la lista in maniera coerente alla spesa crescente sul mese
        for el in month_import_list:
        	if(el == 0):
        		el = previous_period
        	else:
        ...
        plt.plot(month_len,month_import_list)
        self.graphView 
